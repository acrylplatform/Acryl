...
import com.acrylplatform.extensions.{Extension, Context => ExtensionContext}
import com.acrylplatform.settings.GRPCSettings
...
class GRPCServerExtension(context: ExtensionContext) extends Extension with ScorexLogging {
  ...
  override def start(): Unit = {
    val settings = context.settings.config.as[GRPCSettings]("acryl.grpc")
