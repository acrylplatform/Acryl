package com.acrylplatform.lang.v1.evaluator.ctx.impl.acryl
...
import com.acrylplatform.lang.v1.compiler.Types.{BYTESTR, LONG, STRING, _}
...
import com.acrylplatform.lang.v1.traits.domain.{OrdType, Recipient}
...
object AcrylContext {
  ...
  def build(ds: DirectiveSet, env: Environment): CTX = {
    ...
    val txByIdF: BaseFunction = {
      val returnType = com.acrylplatform.lang.v1.compiler.Types.UNION.create(UNIT +: anyTransactionType.typeList)
    ...
    val assetBalanceF: BaseFunction =
      NativeFunction(
        ...
        ("addressOrAlias", addressOrAliasType, "account"),
        ("assetId", UNION(UNIT, BYTESTR), "assetId (ACRYL if none)")
      ) {
    ...
    val acrylBalanceF: UserFunction =
      UserFunction("acrylBalance", 109, LONG, "get ACRYL balance for account", ("@addressOrAlias", addressOrAliasType, "account")) {
    ...
    val commonVars = Map(
      ("height", ((com.acrylplatform.lang.v1.compiler.Types.LONG, "Current blockchain height"), LazyVal(EitherT(heightCoeval)))),
    )
    ...
    val types = buildAcrylTypes(proofsEnabled, version)
