...
import com.acrylplatform.generator.GeneratorSettings.NodeAddress
import com.acrylplatform.generator.Preconditions.{PGenSettings, UniverseHolder}
import com.acrylplatform.generator.cli.ScoptImplicits
import com.acrylplatform.generator.config.FicusImplicits
import com.acrylplatform.generator.utils.Universe
import com.acrylplatform.network.client.NetworkSender
import com.acrylplatform.settings.AcrylSettings
import com.acrylplatform.transaction.Transaction
import com.acrylplatform.utils.{LoggerFacade, NTP}
...
object TransactionsGeneratorApp extends App with ScoptImplicits with FicusImplicits with EnumerationReader {
  ...
  // IDEA bugs
  implicit val inetSocketAddressReader        = com.acrylplatform.settings.inetSocketAddressReader
  ...
  val parser = new OptionParser[GeneratorSettings]("generator") {
    head("TransactionsGenerator - Acryl load testing transactions generator")
  ...
  val acrylSettings = AcrylSettings.fromRootConfig(ConfigFactory.load())
  ...
  parser.parse(args, defaultConfig) match {
    ...
    case Some(finalConfig) =>
      ...
      val sender = new NetworkSender(acrylSettings.networkSettings.trafficLogger, finalConfig.addressScheme, "generator", nonce = Random.nextLong())
