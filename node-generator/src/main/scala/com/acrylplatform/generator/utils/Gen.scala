...
import com.acrylplatform.generator.utils.Implicits._
...
object Gen {
  ...
  def transfers(senderGen: Iterator[KeyPair], recipientGen: Iterator[Address], feeGen: Iterator[Long]): Iterator[Transaction] = {
    ...
    senderGen
      ...
      .map {
        case (((src, dst), fee), i) =>
          TransferTransactionV2.selfSigned(Acryl, src, dst, fee, now + i, Acryl, fee, Array.emptyByteArray)
      }
    ...
    senderGen
      ...
      .map {
        case ((sender, count), i) =>
          ...
          MassTransferTransaction.selfSigned(Acryl, sender, transfers, now + i, fee, Array.emptyByteArray)
      }
