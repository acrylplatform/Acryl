...
import com.acrylplatform.it.api.AsyncHttpApi._
import com.acrylplatform.it.util.GlobalTimer.{instance => timer}
...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false, imageName: String = Docker.NodeImageName)
  ...
  private val logDir: Coeval[Path] = Coeval.evalOnce {
    val r = Option(System.getProperty("acryl.it.logging.dir"))
      .map(Paths.get(_))
  ...
  private lazy val acrylNetwork: Network = {
    val id          = Random.nextInt(Int.MaxValue)
    val networkName = s"acryl-$id"
  ...
  def createNetwork: Network = acrylNetwork
  ...
  private def startNodeInternal(nodeConfig: Config): DockerNode =
    try {
      ...
      val networkPort = actualConfig.getString("acryl.network.port")
      val hostConfig = HostConfig
        .builder()
        .publishAllPorts(true)
        .build()

      val nodeName   = actualConfig.getString("acryl.network.node-name")
      ...
      val configOverrides: String = {
        val ntpServer    = Option(System.getenv("NTP_SERVER")).fold("")(x => s"-Dacryl.ntp-server=$x ")
        val maxCacheSize = Option(System.getenv("MAX_CACHE_SIZE")).fold("")(x => s"-Dacryl.max-cache-size=$x ")

        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dacryl.network.declared-address=$ip:$networkPort $ntpServer $maxCacheSize"
        ...
        if (enableProfiling) {
          ...
          config += s"-agentpath:/usr/local/YourKit-JavaProfiler-2019.1/bin/linux-x86-64/libyjpagent.so=port=$ProfilerPort,listen=all," +
            s"sampling,monitors,sessionname=AcrylNode,dir=$ContainerRoot/profiler,logdir=$ContainerRoot,onexit=snapshot "
        }
      ...
      val containerConfig = ContainerConfig
        .builder()
        .image(imageName)
        .networkingConfig(ContainerConfig.NetworkingConfig.create(Map(acrylNetwork.name() -> endpointConfigFor(nodeName)).asJava))
        .hostConfig(hostConfig)
        .env(s"ACRYL_OPTS=$configOverrides")
        .build()

      val containerId = {
        val containerName = s"${acrylNetwork.name()}-$nodeName"
        dumpContainers(
      ...
      val node = new DockerNode(actualConfig, containerId, getNodeInfo(containerId, AcrylSettings.fromRootConfig(actualConfig)))
      nodes.add(node)
  ...
  private def inspectContainer(containerId: String): ContainerInfo = {
    ...
    if (containerInfo.networkSettings().networks().asScala.contains(acrylNetwork.name())) containerInfo
    else {
      log.debug(s"Container $containerId has not connected to the network ${acrylNetwork.name()} yet, retry")
      Thread.sleep(1000)
  ...
  def restartNode(node: DockerNode, configUpdates: Config = empty): DockerNode = {
    ...
    if (configUpdates != empty) {
      ...
      val shPath = "/opt/acryl/start-acryl.sh"
      val scriptCmd: Array[String] =
        Array("sh", "-c", s"sed -i 's|$$ACRYL_OPTS.*-cp|$$ACRYL_OPTS $renderedConfig -cp|' $shPath && chmod +x $shPath")
  ...
  override def close(): Unit = {
    if (isStopped.compareAndSet(false, true)) {
      ...
      try {
        client.removeNetwork(acrylNetwork.id)
      } catch {
        case NonFatal(e) =>
          // https://github.com/moby/moby/issues/17217
          log.warn(s"Can not remove network ${acrylNetwork.name()}", e)
      }
  ...
  private def disconnectFromNetwork(containerId: String): Unit = {
    ...
    client.disconnectFromNetwork(containerId, acrylNetwork.id())
  }
...
object Docker {
  val NodeImageName: String = "com.acrylplatform/node-it:latest"

  private val ContainerRoot = Paths.get("/opt/acryl")
  ...
  def genesisOverride: Config = {
    ...
    val timestampOverrides = parseString(s"""acryl.blockchain.custom.genesis {
                                            |  timestamp = $genesisTs
    ...
    val gs               = genesisConfig.as[GenesisSettings]("acryl.blockchain.custom.genesis")
    val genesisSignature = Block.genesis(gs).explicitGet().uniqueId

    parseString(s"acryl.blockchain.custom.genesis.signature = $genesisSignature").withFallback(timestampOverrides)
  }

  AddressScheme.current = new AddressScheme {
    override val chainId: Byte = configTemplate.as[String]("acryl.blockchain.custom.address-scheme-character").charAt(0).toByte
  }
