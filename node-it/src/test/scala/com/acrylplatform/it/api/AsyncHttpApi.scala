...
import com.acrylplatform.http.DebugMessage._
import com.acrylplatform.http.{DebugMessage, RollbackParams, api_key}
...
import com.acrylplatform.transaction.assets.{BurnTransaction, IssueTransaction, SetAssetScriptTransaction, SponsorFeeTransaction}
...
import com.acrylplatform.transaction.{CreateAliasTransaction, DataTransaction}
...
object AsyncHttpApi extends Assertions {
  ...
  implicit class NodeAsyncHttpApi(val n: Node) extends Assertions with Matchers {
    ...
    def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long): Future[Transaction] =
      postJson("/acryl/payment", PaymentRequest(amount, fee, sourceAddress, recipient)).as[Transaction]
    ...
    def debugStateAt(height: Long): Future[Map[String, Long]] = getWithApiKey(s"/debug/stateAcryl/$height").as[Map[String, Long]]
