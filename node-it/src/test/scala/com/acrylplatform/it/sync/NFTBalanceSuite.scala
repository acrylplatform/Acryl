...
import com.acrylplatform.it.api._
...
import com.acrylplatform.transaction.assets.{IssueTransaction, IssueTransactionV1}
...
class NFTBalanceSuite
  ...
  protected val theNodes: Coeval[Seq[Node]] = Coeval.evalOnce {
    Option(System.getProperty("acryl.it.config.file")) match {
  ...
  protected override def beforeAll(): Unit = {
    ...
    val fundAndIssue =
      for {
        ...
        fundTx <- node.transfer(node.address, issuer.address, 1000.acryl, 0.001.acryl)
  ...
  val configs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw(s"""
                          |acryl.blockchain.custom.functionality.pre-activated-features = {
                          |          2 = 0
                          |          3 = 0
                          |          4 = 0
                          |          5 = 0
    ...
    val transactions =
      Future.sequence(addrs map { addr =>
        NodeAsyncHttpApi(faucet)
          .transfer(faucet.address, addr, 1000.acryl, 0.001.acryl)
          .flatMap { tx =>
            NodeAsyncHttpApi(faucet)
