...
class UtxSuite extends FunSuite with CancelAfterFailure with NodesFromDocker with Matchers {
  ...
  test("Invalid transaction should be removed from from utx") {
    ...
    val transferToAccount = TransferTransactionV1
      .selfSigned(Acryl, miner.privateKey, account, AMOUNT, System.currentTimeMillis(), Acryl, ENOUGH_FEE, Array.emptyByteArray)
      .explicitGet()
    ...
    val firstTransfer = TransferTransactionV1
      .selfSigned(Acryl, account, miner.privateKey, AMOUNT - ENOUGH_FEE, System.currentTimeMillis(), Acryl, ENOUGH_FEE, Array.emptyByteArray)
      .explicitGet()

    val secondTransfer = TransferTransactionV1
      .selfSigned(Acryl, account, notMiner.privateKey, AMOUNT - ENOUGH_FEE, System.currentTimeMillis(), Acryl, ENOUGH_FEE, Array.emptyByteArray)
      .explicitGet()
