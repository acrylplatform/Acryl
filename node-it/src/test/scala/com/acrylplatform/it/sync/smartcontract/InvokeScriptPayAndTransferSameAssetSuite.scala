...
import com.acrylplatform.it.sync.{setScriptFee, smartFee, smartMinFee}
...
class InvokeScriptPayAndTransferSameAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_set script to dApp account and transfer out all acryl") {
    ...
    val dAppScript        = ScriptCompiler.compile(s"""
          ...
          |  else throw("need payment in ACRYL or any Asset")
          |}
  ...
  test("dApp can transfer payed Acryl if its own balance is 0") {
    dAppInitBalance = sender.accountBalances(dApp)._1
    callerInitBalance = sender.accountBalances(caller)._1
    receiverInitBalance = sender.accountBalances(receiver)._1
  ...
  def invoke(func: String, amount: Long, asset: Asset = Acryl, fee: Long = 500000): String = {
    sender
      .invokeScript(
        caller,
        dApp,
