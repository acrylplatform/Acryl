...
import com.acrylplatform.it.api.{DataResponse, DebugStateChanges, StateChangesDetails, TransactionInfo, TransfersInfoResponse}
import com.acrylplatform.it.api.SyncHttpApi._
import com.acrylplatform.it.sync.setScriptFee
import com.acrylplatform.it.transactions.BaseTransactionSuite
import com.acrylplatform.it.util._
import com.acrylplatform.lang.v1.compiler.Terms.{CONST_LONG, CONST_STRING}
...
class InvokeScriptTransactionStateChangesSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("prepare") {
    ...
    sender.massTransfer(contract.address, List(Transfer(caller.address, 3000), Transfer(recipient.address, 3000)), 0.01.acryl, Some(simpleAsset))
    sender.massTransfer(contract.address, List(Transfer(caller.address, 3000), Transfer(recipient.address, 3000)), 0.01.acryl, Some(assetSponsoredByDApp))
    sender.massTransfer(recipient.address, List(Transfer(caller.address, 3000), Transfer(contract.address, 3000)), 0.01.acryl, Some(assetSponsoredByRecipient))
    ...
    val script = ScriptCompiler.compile(
      """
        ...
        |
        |@Callable(i)
        |func writeAndSendAcryl(value: Int, recipient: String, amount: Int) = {
        |    ScriptResult(
        |        WriteSet([DataEntry("result", value)]),
