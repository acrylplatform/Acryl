...
import com.acrylplatform.it.sync.{issueFee, minFee, smartFee, smartMinFee}
...
class InvokeScriptWithSmartAccountAndAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_send acryl to dApp and caller accounts") {
    val dAppTransferId        = sender.transfer(sender.address, dApp.address, 5.acryl, minFee).id
    val callerTransferId      = sender.transfer(sender.address, caller.address, 5.acryl, minFee).id
    val smartCallerTransferId = sender.transfer(sender.address, smartCaller.address, 5.acryl, minFee).id
  ...
  test("invoke by smart account requires just 1 extra fee") {
    assertBadRequestAndMessage(
      sender.invokeScript(
        ...
        dApp.address,
        Some("justWriteData"),
        fee = 0.00899999.acryl
      ),
      s"does not exceed minimal value of 900000 ACRYL"
    )
  }

  test("max fee is 0.053 Acryl (0.005 + extraFee(1 smart caller + 1 payment + 10 transfers))") {
  ...
  test("can't invoke with small fee for both smart assets") {
    ...
    assertBadRequestAndMessage(
      sender
        ...
        .id,
      "does not exceed minimal value of 900000 ACRYL"
    )
  }
