...
class RIDEFuncSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("assetBalance() verification") {
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        TransferTransactionV2
          .selfSigned(Acryl, pkNewAddress, pkNewAddress, 1.acryl, System.currentTimeMillis(), Acryl, smartMinFee, Array())
          .explicitGet()
          .json()),
    ...
    val udpatedScript =
      s"""
         |match tx {
         |  case tx : SetScriptTransaction => true
         |  case other => assetBalance(tx.sender, base58'$asset') >= 900000000 && acrylBalance(tx.sender) >500000000
         |}
      """.stripMargin
  ...
  test("lastBlock and blockInfoByHeight(last) must return liquid block") {
    ...
    val transfer = sender.transfer(newAddress, newAddress, 1.acryl, minFee + (2 * smartFee))
    nodes.waitForHeightAriseAndTxPresent(transfer.id)
  }
}
