...
class SetScriptTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("setup acc0 with 1 acryl") {
    sender.transfer(
      sender.address,
      acc0.address,
      assetId = None,
      amount = 3 * transferAmount + 3 * (0.00001.acryl + 0.00002.acryl), // Script fee
      fee = minFee,
      version = 2,
      waitForTx = true
    )
  ...
  test("can't send from acc0 using old pk") {
    assertBadRequest(
      sender.transfer(
        ...
        fee = minFee + 0.00001.acryl + 0.00002.acryl,
      ))
  }
