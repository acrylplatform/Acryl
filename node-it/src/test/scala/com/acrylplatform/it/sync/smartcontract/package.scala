...
import com.acrylplatform.transaction.assets.exchange.{AssetPair, ExchangeTransactionV2, Order}
...
package object smartcontract {
  ...
  def acrylContextScript(dtx: DataTransaction, accountScript: Boolean): String =
    s"""
       | match tx {
       |  case ext : ExchangeTransaction =>
       |    # Acryl context
       ...
       |
       |     let balances = assetBalance(ext.sender, unit) > 0 && acrylBalance(ext.sender) != 0
       |
