...
import com.acrylplatform.it.sync.{calcDataFee, minFee}
import com.acrylplatform.it.transactions.BaseTransactionSuite
import com.acrylplatform.it.util._
import com.acrylplatform.state.{BinaryDataEntry, BooleanDataEntry, DataEntry, IntegerDataEntry, StringDataEntry}
...
class DataTransactionSuite extends BaseTransactionSuite {
  ...
  test("sender's acryl balance is decreased by fee.") {
  ...
  test("cannot transact without having enough acryl") {
    val (balance1, eff1) = miner.accountBalances(firstAddress)

    val data = List(BooleanDataEntry("bool", false))
    assertBadRequestAndResponse(sender.putData(firstAddress, data, balance1 + 1), "negative acryl balance")
    nodes.waitForHeightArise()
    miner.assertBalances(firstAddress, balance1, eff1)

    val leaseAmount = 1.acryl
  ...
  test("try to send tx above limits of key, value and size") {
    ...
    assertBadRequestAndResponse(sender.putData(firstAddress, extraValueData, 1.acryl), TooBig)
    nodes.waitForHeightArise()

    val largeBinData = List.tabulate(5)(n => BinaryDataEntry(extraKey, ByteStr(Array.fill(MaxValueSize)(n.toByte))))
    assertBadRequestAndResponse(sender.putData(firstAddress, largeBinData, 1.acryl), TooBig)
    nodes.waitForHeightArise()

    val largeStrData = List.tabulate(5)(n => StringDataEntry(extraKey, "A" * MaxValueSize))
    assertBadRequestAndResponse(sender.putData(firstAddress, largeStrData, 1.acryl), TooBig)
    nodes.waitForHeightArise()

    val tooManyEntriesData = List.tabulate(MaxEntryCount + 1)(n => IntegerDataEntry("key", 88))
    assertBadRequestAndResponse(sender.putData(firstAddress, tooManyEntriesData, 1.acryl), TooBig)
    nodes.waitForHeightArise()
  }
