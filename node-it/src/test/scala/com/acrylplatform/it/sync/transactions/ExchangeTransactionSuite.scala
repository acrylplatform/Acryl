...
import com.acrylplatform.it.sync.smartcontract.exchangeTx
...
class ExchangeTransactionSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("cannot exchange non-issued assets") {
    for ((o1ver, o2ver, tver) <- versions) {
      ...
      val buyAmount           = 1
      val sellAmount          = 1
      pair = AssetPair.createAssetPair("ACRYL", assetId).get
  ...
  test("negative - check orders v2 and v3 with exchange tx v1") {
    ...
    pair = AssetPair.createAssetPair("ACRYL", exchAsset.id().base58).get
  ...
  test("exchange tx with orders v3") {
    ...
    for ((o1ver, o2ver, matcherFeeOrder1, matcherFeeOrder2) <- Seq(
           (1: Byte, 3: Byte, Acryl, IssuedAsset(assetId)),
           (1: Byte, 3: Byte, Acryl, Acryl),
           (2: Byte, 3: Byte, Acryl, IssuedAsset(assetId)),
           (3: Byte, 1: Byte, IssuedAsset(assetId), Acryl),
           (2: Byte, 3: Byte, Acryl, Acryl),
           (3: Byte, 2: Byte, IssuedAsset(assetId), Acryl)
         )) {
      ...
      if (matcherFeeOrder1 == Acryl && matcherFeeOrder2 != Acryl) {
      ...
      val assetPair  = AssetPair.createAssetPair("ACRYL", assetId.base58).get
