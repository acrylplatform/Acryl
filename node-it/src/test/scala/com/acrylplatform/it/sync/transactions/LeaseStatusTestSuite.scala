...
import com.acrylplatform.transaction.lease.LeaseTransaction.Status.{Active, Canceled}
...
object LeaseStatusTestSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""acryl {
       |   miner{
       |      enable = yes
  ...
  private val notMinerConfig = ConfigFactory.parseString(s"""acryl {
       |   miner.enable = no
