...
import com.acrylplatform.api.http.assets.{MassTransferRequest, SignedMassTransferRequest}
...
import com.acrylplatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
import com.acrylplatform.transaction.transfer.TransferTransaction.MaxAttachmentSize
...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("asset mass transfer changes asset balances and sender's.acryl balance is decreased by fee.") {
  ...
  test("acryl mass transfer changes acryl balances") {
    val (balance1, eff1) = miner.accountBalances(firstAddress)
    val (balance2, eff2) = miner.accountBalances(secondAddress)
  ...
  test("can not make mass transfer without having enough acryl") {
    val (balance1, eff1) = miner.accountBalances(firstAddress)
    val (balance2, eff2) = miner.accountBalances(secondAddress)
    val transfers        = List(Transfer(secondAddress, balance1 / 2), Transfer(thirdAddress, balance1 / 2))

    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative acryl balance")
  ...
  test("invalid transfer should not be in UTX or blockchain") {
    ...
    def request(transfers: List[Transfer] = List(Transfer(secondAddress, transferAmount)),
      ...
      val txEi = for {
        ...
        tx              <- MassTransferTransaction.selfSigned(Acryl, sender.privateKey, parsedTransfers, timestamp, fee, attachment)
      } yield tx
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.acryl), Transfer(secondAddress, 2.acryl), Transfer(thirdAddress, 3.acryl))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.acryl)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.acryl)
  ...
  test("reporting MassTransfer transactions to aliases") {
    ...
    val transfers = aliases.map { alias =>
      Transfer(Alias.create(alias).explicitGet().stringRepr, 2.acryl)
    }
    val txId = sender.massTransfer(firstAddress, transfers, 300000).id
    nodes.waitForHeightAriseAndTxPresent(txId)
