...
import com.acrylplatform.it.sync.{script, someAssetAmount, _}
...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("cannot transact without having enough acryl") {
    val (balance, eff) = miner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.setAssetScript(assetWScript, firstAddress, balance + 1, Some(scriptBase64)), "negative acryl balance")
