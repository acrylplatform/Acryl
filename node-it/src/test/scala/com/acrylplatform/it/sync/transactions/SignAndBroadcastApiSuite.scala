...
import com.acrylplatform.api.http.assets.SignedTransferV1Request
import com.acrylplatform.common.state.ByteStr
import com.acrylplatform.common.utils.{Base58, EitherExt2}
import com.acrylplatform.crypto
import com.acrylplatform.it.NTPTime
...
import com.acrylplatform.transaction.assets.exchange.AssetPair.extractAssetId
import com.acrylplatform.transaction.assets.exchange.{AssetPair, Order, _}
import com.acrylplatform.transaction.assets.{BurnTransaction, IssueTransaction, ReissueTransaction, SponsorFeeTransaction}
...
class SignAndBroadcastApiSuite extends BaseTransactionSuite with NTPTime {
  test("height should always be reported for transactions") {
    val txId = sender.transfer(firstAddress, secondAddress, 1.acryl, fee = minFee).id
  ...
  test("/transactions/sign should produce issue/reissue/burn/transfer transactions that are good for /transactions/broadcast") {
    for (v <- supportedVersions) {
      ...
      signBroadcastAndCalcFee(
        Json.obj("type" -> ReissueTransaction.typeId, "quantity" -> 200.acryl, "assetId" -> issueId, "sender" -> firstAddress, "reissuable" -> false),
        usesProofs = isProof,
        version = v
      )
      ...
      signBroadcastAndCalcFee(Json.obj("type" -> BurnTransaction.typeId, "quantity" -> 100.acryl, "assetId" -> issueId, "sender" -> firstAddress),
                              usesProofs = isProof,
                              version = v)
  ...
  test("/transactions/sign should produce mass transfer transaction that is good for /transactions/broadcast") {
    signBroadcastAndCalcFee(
      Json.obj(
        ...
        "version"    -> 1,
        "sender"     -> firstAddress,
        "transfers"  -> Json.toJson(Seq(Transfer(secondAddress, 1.acryl), Transfer(thirdAddress, 2.acryl))),
  ...
  test("/transactions/broadcast should produce ExchangeTransaction with custom asset") {
    ...
    val versionsWithAcrylFee =
      (transactionV1versions +: transactionV2versions)
        .map { case (o1ver, o2ver, tver) => (o1ver, o2ver, tver, Acryl, Acryl) }

    val versionsWithAssetFee = for {
      o2ver <- 1 to 3
      buyMatcherFeeAssetId  = assetId
      sellMatcherFeeAssetId = Acryl
    } yield (3.toByte, o2ver.toByte, 2.toByte, buyMatcherFeeAssetId, sellMatcherFeeAssetId)

    for ((o1ver, o2ver, tver, matcherFeeOrder1, matcherFeeOrder2) <- versionsWithAcrylFee ++ versionsWithAssetFee) {
      ...
      val buyAmount           = 2
      val sellAmount          = 3
      val assetPair           = AssetPair.createAssetPair("ACRYL", issueTx).get
