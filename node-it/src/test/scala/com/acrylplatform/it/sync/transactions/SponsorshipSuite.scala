...
import com.acrylplatform.it.{NodeConfigs, ReportingTestName}
...
class SponsorshipSuite extends FreeSpec with NodesFromDocker with Matchers with ReportingTestName with CancelAfterFailure {
  ...
  override def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("acryl.blockchain.custom.functionality.blocks-for-feature-activation=1"))
      .overrideBase(_.raw("acryl.blockchain.custom.functionality.feature-check-blocks-period=1"))
      .withDefault(1)
  ...
  "Fee in sponsored asset works fine" - {
    ...
    val sponsorAcrylBalance = sponsor.accountBalances(sponsor.address)._2
    val minerAcrylBalance   = miner.accountBalances(miner.address)._2
    ...
    "check balance before test accounts balances" in {
      ...
      sponsor.assertBalances(sponsor.address, sponsorAcrylBalance - 2 * issueFee - minFee)
    ...
    val minerAcrylBalanceAfterFirstXferTest   = minerAcrylBalance + 2.acryl + minFee + CommonValidation.FeeUnit * SmallFee / minSponsorFee
    val sponsorAcrylBalanceAfterFirstXferTest = sponsorAcrylBalance - 2.acryl - minFee - CommonValidation.FeeUnit * SmallFee / minSponsorFee
...
//        sponsorTx.head.count(tx => tx.sender.contains(sponsor.address) || tx.recipient.contains(sponsor.address)) shouldBe 4
      ...
      "sponsor should receive sponsored asset as fee, acryl should be written off" in {
        miner.assertAssetBalance(sponsor.address, sponsorAssetId, sponsorAssetTotal / 2 + SmallFee)
        miner.assertBalances(sponsor.address, sponsorAcrylBalanceAfterFirstXferTest)
      }

      "miner acryl balance should be changed" in {
        miner.assertBalances(miner.address, minerAcrylBalanceAfterFirstXferTest)
      }
    }

    "assets balance should contain sponsor fee info and sponsor balance" in {
      val sponsorLeaseSomeAcryl = sponsor.lease(sponsor.address, bob.address, leasingAmount, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseSomeAcryl)
    ...
    "acryl fee depends on sponsor fee and sponsored token decimals" in {
      val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.acryl, LargeFee, None, Some(sponsorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
      assert(!transferTxCustomFeeAlice.isEmpty)
      ...
      miner.assertBalances(
        sponsor.address,
        sponsorAcrylBalanceAfterFirstXferTest - CommonValidation.FeeUnit * LargeFee / Token - leasingFee,
        sponsorAcrylBalanceAfterFirstXferTest - CommonValidation.FeeUnit * LargeFee / Token - leasingFee - leasingAmount
      )
      miner.assertBalances(miner.address, minerAcrylBalanceAfterFirstXferTest + CommonValidation.FeeUnit * LargeFee / Token + leasingFee)
    }
    ...
    "cancel sponsorship" - {
      ...
      "check sponsor and miner balances after cancel" in {
        miner.assertBalances(
          sponsor.address,
          sponsorAcrylBalanceAfterFirstXferTest - CommonValidation.FeeUnit * LargeFee / Token - leasingFee - issueFee,
          sponsorAcrylBalanceAfterFirstXferTest - CommonValidation.FeeUnit * LargeFee / Token - leasingFee - leasingAmount - issueFee
        )
        miner.assertBalances(miner.address, minerAcrylBalanceAfterFirstXferTest + CommonValidation.FeeUnit * LargeFee / Token + leasingFee + issueFee)
      }
    ...
    "set sponsopship again" - {
      ...
      "make transfer with new min sponsored fee" in {
        ...
        val aliceAcrylBalance   = alice.accountBalances(alice.address)
        val bobAssetBalance     = bob.assetBalance(bob.address, sponsorAssetId).balance
        val bobAcrylBalance     = bob.accountBalances(bob.address)
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.acryl, TinyFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        nodes.waitForHeightArise()

        val acrylFee = CommonValidation.FeeUnit * TinyFee / TinyFee
        sponsor.assertBalances(sponsor.address, sponsoredBalance._1 - acrylFee, sponsoredBalance._2 - acrylFee)
        ...
        alice.assertBalances(alice.address, aliceAcrylBalance._2 - 1.acryl)
        bob.assertBalances(bob.address, bobAcrylBalance._1 + 1.acryl, bobAcrylBalance._2 + 1.acryl)
        bob.assertAssetBalance(bob.address, sponsorAssetId, bobAssetBalance)
        miner.assertBalances(miner.address, minerBalance._2 + acrylFee)
      ...
      "transfer tx sponsored fee is less then new minimal" in {
        assertBadRequestAndResponse(
          sponsor
            ...
            .id,
          s"Fee for TransferTransaction \\($SmallFee in ${Some(sponsorAssetId).get}\\) does not exceed minimal value of 100000 ACRYL or $LargeFee ${Some(sponsorAssetId).get}"
        )
      }
      ...
      "make transfer with updated min sponsored fee" in {
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.acryl, LargeFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        val acrylFee = CommonValidation.FeeUnit * LargeFee / LargeFee
        nodes.waitForHeightArise()
    ...
    "issue asset make sponsor and burn and reissue" in {
      ...
      val aliceTransferAcryl = alice.transfer(alice.address, bob.address, transferAmount, SmallFee, None, Some(sponsorAssetId2)).id
      nodes.waitForHeightAriseAndTxPresent(aliceTransferAcryl)
      nodes.waitForHeightArise()

      val totalAcrylFee = CommonValidation.FeeUnit * SmallFee / Token + issueFee + sponsorFee + burnFee + minFee + issueFee
      miner.assertBalances(miner.address, minerBalance._1 + totalAcrylFee)
      sponsor.assertBalances(sponsor.address, sponsorBalance._1 - totalAcrylFee, sponsorBalance._2 - totalAcrylFee)
    ...
    "miner is sponsor" in {
      ...
      val aliceSponsoredTransferAcryl = alice.transfer(alice.address, bob.address, transferAmount, SmallFee, None, Some(minersSpondorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(aliceSponsoredTransferAcryl)
      nodes.waitForHeightArise()
    ...
    "tx is declined if sponsor has not enough effective balance to pay fee" in {
      ...
      val sponsorLeaseAllAvaliableAcryl             = sponsor.lease(sponsor.address, bob.address, sponsorEffectiveBalance - leasingFee, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseAllAvaliableAcryl)
      assertBadRequestAndMessage(alice.transfer(alice.address, bob.address, 10 * Token, LargeFee, Some(sponsorAssetId), Some(sponsorAssetId)),
                                 "unavailable funds")
      val cancelLeasingTx = sponsor.cancelLease(sponsor.address, sponsorLeaseAllAvaliableAcryl, leasingFee).id
