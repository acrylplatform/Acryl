...
inTask(assembly)(
  Seq(
    test := {},
    assemblyJarName := s"acryl-all-${version.value}.jar",
...
// Logback creates a "acryl.directory_UNDEFINED" without this option.
bashScriptExtraDefines ++= Seq(
  s"""addJava "-Dacryl.defaults.directory=/var/lib/${(Universal / normalizedName).value}"""",
  s"""addJava "-Dacryl.defaults.config.directory=/etc/${(Universal / normalizedName).value}""""
)

inConfig(Universal)(
  Seq(
    mappings += (baseDirectory.value / s"acryl-${network.value}.conf" -> "doc/acryl.conf.sample"),
    mappings := {
...
inConfig(Linux)(
  Seq(
    ...
    packageSummary := "Acryl node",
    packageDescription := "Acryl node"
  ))
...
moduleName := s"acryl${network.value.packageSuffix}" // acryl-*.jar instead of node-*.jar
executableScriptName := moduleName.value // bin/acryl instead of bin/node
