...
object Explorer extends ScorexLogging {
  ...
  def main(args: Array[String]): Unit = {
    val configFilename = Try(args(0)).toOption.getOrElse("acryl-testnet.conf")

    val settings = AcrylSettings.fromRootConfig(loadConfig(ConfigFactory.parseFile(new File(configFilename))))
      ...
      flag match {
        ...
        case "A" =>
          ...
          val balances = wbh.map { h =>
            val k = Keys.acrylBalance(addressId)(h)
        ...
        case "AP" =>
          ...
          log.info(s"$address : ${portfolio.balance} ACRYL, ${portfolio.lease}, ${portfolio.assets.size} assets")
