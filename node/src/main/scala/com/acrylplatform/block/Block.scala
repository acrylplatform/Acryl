...
import com.acrylplatform.block.fields.FeaturesBlockField
import com.acrylplatform.common.state.ByteStr
import com.acrylplatform.common.utils.EitherExt2
import com.acrylplatform.consensus.nxt.{NxtConsensusBlockField, NxtLikeConsensusBlockData}
import com.acrylplatform.crypto
import com.acrylplatform.crypto._
import com.acrylplatform.lang.ValidationError
import com.acrylplatform.settings.GenesisSettings
...
case class Block private[block] (override val timestamp: Long,
  ...
  val json: Coeval[JsObject] = Coeval.evalOnce(
    BlockHeader.json(this, bytes().length) ++
      Json.obj("fee" -> transactionData.map(_.assetFee).collect { case (Acryl, feeAmt) => feeAmt }.sum) ++
  ...
  val feesPortfolio: Coeval[Portfolio] = Coeval.evalOnce(Monoid[Portfolio].combineAll({
    ...
    case (assetId, feeVolume) =>
      assetId match {
        case Acryl                  => Portfolio(feeVolume, LeaseBalance.empty, Map.empty)
