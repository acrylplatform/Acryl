...
import com.acrylplatform.settings.DBSettings
...
import com.acrylplatform.utils.{ObservedLoadingCache, ScorexLogging}
...
abstract class Caches(spendableBalanceChanged: Observer[(Address, Asset)]) extends Blockchain with ScorexLogging {
  ...
  def append(diff: Diff, carryFee: Long, totalFee: Long, block: Block): Unit = {
    ...
    val acrylBalances        = Map.newBuilder[BigInt, Long]
    ...
    for ((address, portfolioDiff) <- diff.portfolios) {
      ...
      if (portfolioDiff.balance != 0) {
        val wbalance = portfolioDiff.balance + balance(address, Acryl)
        acrylBalances += aid            -> wbalance
        newBalances += (address, Acryl) -> wbalance
      }
