...
import com.acrylplatform.lang.script.{Script, ScriptReader}
...
object Keys {
  ...
  def acrylBalanceHistory(addressId: BigInt): Key[Seq[Int]] = historyKey("acryl-balance-history", 5, addressId.toByteArray)
  def acrylBalance(addressId: BigInt)(height: Int): Key[Long] =
    Key("acryl-balance", hAddr(6, height, addressId), Option(_).fold(0L)(Longs.fromByteArray), Longs.toByteArray)
  ...
  val addressesForAcrylSeqNr: Key[Int]                = intKey("addresses-for-acryl-seq-nr", 37)
  def addressesForAcryl(seqNr: Int): Key[Seq[BigInt]] = Key("addresses-for-acryl", h(38, seqNr), readBigIntSeq, writeBigIntSeq)
  ...
  val acrylAmountPrefix: Short              = 58
  def acrylAmount(height: Int): Key[BigInt] = Key("acryl-amount", h(acrylAmountPrefix, height), Option(_).fold(BigInt(0))(BigInt(_)), _.toByteArray)
}
