...
import com.acrylplatform.database.patch.DisableHijackedAliases
...
import com.acrylplatform.transaction.Transaction.Type
import com.acrylplatform.transaction.TxValidationError.{AliasDoesNotExist, AliasIsDisabled, GenericError}
...
import com.acrylplatform.utils.{CloseableIterator, Paged, ScorexLogging}
...
class LevelDBWriter(writableDB: DB, spendableBalanceChanged: Observer[(Address, Asset)], val settings: BlockchainSettings, val dbSettings: DBSettings)
  ...
  protected override def loadBalance(req: (Address, Asset)): Long = readOnly { db =>
    addressId(req._1).fold(0L) { addressId =>
      req._2 match {
        ...
        case Acryl                  => db.fromHistory(Keys.acrylBalanceHistory(addressId), Keys.acrylBalance(addressId)).getOrElse(0L)
      }
    }
  }
  ...
  private def loadLposPortfolio(db: ReadOnlyDB, addressId: BigInt) = Portfolio(
    db.fromHistory(Keys.acrylBalanceHistory(addressId), Keys.acrylBalance(addressId)).getOrElse(0L),
  ...
  override protected def doAppend(block: Block,
    ...
    val newAddressesForAcryl = ArrayBuffer.empty[BigInt]
    val updatedBalanceAddresses = for ((addressId, balance) <- acrylBalances) yield {
      val kwbh = Keys.acrylBalanceHistory(addressId)
      val wbh  = rw.get(kwbh)
      if (wbh.isEmpty) {
        newAddressesForAcryl += addressId
      }
      rw.put(Keys.acrylBalance(addressId)(height), balance)
      expiredKeys ++= updateHistory(rw, wbh, kwbh, balanceThreshold, Keys.acrylBalance(addressId))
      addressId
    }

    val changedAddresses = addressTransactions.keys ++ updatedBalanceAddresses

    if (newAddressesForAcryl.nonEmpty) {
      val newSeqNr = rw.get(Keys.addressesForAcrylSeqNr) + 1
      rw.put(Keys.addressesForAcrylSeqNr, newSeqNr)
      rw.put(Keys.addressesForAcryl(newSeqNr), newAddressesForAcryl)
    }
      ...
      val discardedBlocks: Seq[Block] = for (currentHeight <- height until targetHeight by -1) yield {
        ...
        val discardedBlock = readWrite { rw =>
          ...
          for (aId <- rw.get(Keys.changedAddresses(currentHeight))) {
            ...
            balancesToInvalidate += (address -> Acryl)
            ...
            rw.delete(Keys.acrylBalance(addressId)(currentHeight))
            rw.filterHistory(Keys.acrylBalanceHistory(addressId), currentHeight)
  ...
  override def balanceSnapshots(address: Address, from: Int, to: BlockId): Seq[BalanceSnapshot] = readOnly { db =>
    db.get(Keys.addressId(address)).fold(Seq(BalanceSnapshot(1, 0, 0, 0))) { addressId =>
      ...
      val wbh      = slice(db.get(Keys.acrylBalanceHistory(addressId)), from, toHeigth)
      val lbh      = slice(db.get(Keys.leaseBalanceHistory(addressId)), from, toHeigth)
      for {
        (wh, lh) <- merge(wbh, lbh)
        wb = balanceAtHeightCache.get((wh, addressId), () => db.get(Keys.acrylBalance(addressId)(wh)))
  ...
  override def acrylDistribution(height: Int): Either[ValidationError, Map[Address, Long]] = readOnly { db =>
    val canGetAfterHeight = db.get(Keys.safeRollbackHeight)

    def createMap() =
      (for {
        seqNr     <- (1 to db.get(Keys.addressesForAcrylSeqNr)).par
        addressId <- db.get(Keys.addressesForAcryl(seqNr)).par
        history = db.get(Keys.acrylBalanceHistory(addressId))
        actualHeight <- history.partition(_ > height)._2.headOption
        balance = db.get(Keys.acrylBalance(addressId)(actualHeight))
        if balance > 0
      } yield db.get(Keys.idToAddress(addressId)) -> balance).toMap.seq
