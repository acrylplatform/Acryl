...
import com.acrylplatform.api.http.ApiError.DiscontinuedApi
import com.acrylplatform.api.http.assets.TransferV1Request
import com.acrylplatform.api.http.{ApiRoute, WithSettings}
...
@Api(value = "acryl")
@Deprecated
case class AcrylApiRoute(settings: RestAPISettings, wallet: Wallet, utx: UtxPool, allChannels: ChannelGroup, time: Time)
    extends ApiRoute
    with BroadcastRoute
    with WithSettings {

  override lazy val route = pathPrefix("acryl") {
