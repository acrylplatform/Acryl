...
import com.acrylplatform.account.{Address, AddressScheme, PublicKey}
import com.acrylplatform.common.state.ByteStr
import com.acrylplatform.lang.ValidationError
import com.acrylplatform.lang.script.ScriptReader
import com.acrylplatform.protobuf.transaction.Transaction.Data
import com.acrylplatform.protobuf.transaction.{Script => PBScript}
...
import com.acrylplatform.{transaction => vt}

object PBTransactions {
  import com.acrylplatform.protobuf.utils.PBInternalImplicits._

  def create(sender: com.acrylplatform.account.PublicKey = PublicKey.empty,
             chainId: Byte = 0,
             fee: Long = 0L,
             feeAssetId: VanillaAssetId = Acryl,
             timestamp: Long = 0L,
             version: Int = 0,
             proofsArray: Seq[com.acrylplatform.common.state.ByteStr] = Nil,
             data: com.acrylplatform.protobuf.transaction.Transaction.Data = com.acrylplatform.protobuf.transaction.Transaction.Data.Empty)
    : SignedTransaction = {
    ...
    val result: Either[ValidationError, VanillaTransaction] = data match {
      ...
      case Data.CreateAlias(CreateAliasTransactionData(alias)) =>
        version match {
          case 1 =>
            for {
              alias <- com.acrylplatform.account.Alias.createWithChainId(alias, chainId)
        ...
        version match {
          case 1 =>
            vt.CreateAliasTransactionV1(sender,
                                        com.acrylplatform.account.Alias.createWithChainId(alias, chainId).explicitGet(),
                                        feeAmount,
                                        timestamp,
                                        signature)
  ...
  def protobuf(tx: VanillaTransaction): PBSignedTransaction = {
    ...
    tx match {
      ...
      case vt.PaymentTransaction(sender, recipient, amount, fee, timestamp, signature) =>
        ...
        PBTransactions.create(sender, chainId, fee, Acryl, timestamp, 1, Seq(signature), Data.Payment(data))
  ...
  def toVanillaDataEntry(de: DataTransactionData.DataEntry): com.acrylplatform.state.DataEntry[_] = {
  ...
  def toPBDataEntry(de: com.acrylplatform.state.DataEntry[_]): DataTransactionData.DataEntry = {
