...
package object settings {
  ...
  def loadConfig(maybeUserConfig: Option[Config]): Config = {
    ...
    val cmdDefaults =
      Try(withApp.getConfig("acryl.defaults"))
        .getOrElse(ConfigFactory.empty())
        .atPath("acryl")

    external
      .withFallback(cmdDefaults)
      .withFallback(ConfigFactory.parseString(s"acryl.directory = ${defaultDirectory(withApp)}"))
      .withFallback(ConfigFactory.defaultApplication())
      .withFallback(ConfigFactory.defaultReference())
      .resolve()
  }
  ...
  def defaultDirectory(config: Config): String = {
    ...
    def withNetwork(config: Config): String = {
      val bc = config.getString("acryl.blockchain.type")
      val suffix =
        if (bc == "CUSTOM") {
          val char = config.getString("acryl.blockchain.custom.address-scheme-character").headOption.getOrElse(0.toChar)
