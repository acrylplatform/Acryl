...
import com.acrylplatform.block.{Block, BlockHeader, MicroBlock}
...
import com.acrylplatform.metrics.{TxsInBlockchainStats, _}
import com.acrylplatform.mining.{MiningConstraint, MiningConstraints, MultiDimensionalMiningConstraint}
import com.acrylplatform.settings.{BlockchainSettings, AcrylSettings}
import com.acrylplatform.state.diffs.BlockDiffer
import com.acrylplatform.state.reader.{CompositeBlockchain, LeaseDetails}
import com.acrylplatform.transaction.Asset.IssuedAsset
import com.acrylplatform.transaction.TxValidationError.{BlockAppendError, GenericError, MicroBlockAppendError}
...
import com.acrylplatform.utils.{CloseableIterator, ScorexLogging, Time, UnsupportedFeature, forceStopApplication}
import kamon.Kamon
import monix.reactive.subjects.ReplaySubject
import monix.reactive.{Observable, Observer}

class BlockchainUpdaterImpl(blockchain: LevelDBWriter, spendableBalanceChanged: Observer[(Address, Asset)], AcrylSettings: AcrylSettings, time: Time)
    extends BlockchainUpdater
    with NG
    with ScorexLogging {

  import com.acrylplatform.state.BlockchainUpdaterImpl._
  import AcrylSettings.blockchainSettings.functionalitySettings
  ...
  private lazy val maxBlockReadinessAge = AcrylSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis
  ...
  override val settings: BlockchainSettings = AcrylSettings.blockchainSettings
  ...
  private def featuresApprovedWithBlock(block: Block): Set[Short] = {
    ...
    if (height % featuresCheckPeriod == 0) {
      ...
      if (unimplementedActivated.nonEmpty) {
        ...
        if (AcrylSettings.featuresSettings.autoShutdownOnUnsupportedFeature) {
  ...
  override def processBlock(block: Block, verify: Boolean = true): Either[ValidationError, Option[DiscardedTransactions]] = writeLock {
    ...
    Either
      .cond(
        !AcrylSettings.featuresSettings.autoShutdownOnUnsupportedFeature || notImplementedFeatures.isEmpty,
        (),
          ...
          _ map {
            case (BlockDiffer.Result(newBlockDiff, carry, totalFee, updatedTotalConstraint), discarded) =>
              ...
              if ((block.timestamp > time
                    .getTimestamp() - AcrylSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis) || (height % 100 == 0)) {
  ...
  override def acrylDistribution(height: Int): Either[ValidationError, Map[Address, Long]] = readLock {
    ngState.fold(blockchain.acrylDistribution(height)) { _ =>
      val innerDistribution = blockchain.acrylDistribution(height)
  ...
  override def collectLposPortfolios[A](pf: PartialFunction[(Address, Portfolio), A]): Map[Address, A] = readLock {
    ngState.fold(blockchain.collectLposPortfolios(pf)) { ng =>
      ...
      for ((a, p) <- ng.bestLiquidDiff.portfolios if p.lease != LeaseBalance.empty || p.balance != 0) {
        pf.runWith(b += a -> _)(a -> this.acrylPortfolio(a))
      }
