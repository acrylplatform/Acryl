...
import com.acrylplatform.account.{Address, Alias, PublicKey}
...
object Sponsorship {
  ...
  def calcAcrylFeeAmount(tx: Transaction, getSponsorship: IssuedAsset => Option[Long]): Long = tx.assetFee match {
    case (asset @ IssuedAsset(_), amountInAsset) =>
      val sponsorship = getSponsorship(asset).getOrElse(0L)
      Sponsorship.toAcryl(amountInAsset, sponsorship)

    case (Asset.Acryl, amountInAcryl) =>
      amountInAcryl
  }
