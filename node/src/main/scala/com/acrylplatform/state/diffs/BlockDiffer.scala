...
import com.acrylplatform.state.patch.{CancelAllLeases, CancelInvalidLeaseIn, CancelLeaseOverflow}
import com.acrylplatform.state.reader.CompositeBlockchain
import com.acrylplatform.transaction.{Asset, Transaction}
import com.acrylplatform.transaction.TxValidationError.{ActivationError, _}
...
object BlockDiffer extends ScorexLogging {
  ...
  def fromBlockTraced[Constraint <: MiningConstraint](blockchain: Blockchain,
    ...
    val minerRewardDistr: Portfolio =
      blockchain.lastBlockReward.map(Portfolio.build(Asset.Acryl, _)).getOrElse(Portfolio.empty)
            ...
            balance = portfolio.balance +
              portfolio.assets.map {
                case (assetId, assetFee) =>
                  ...
                  Sponsorship.toAcryl(assetFee, baseFee)
              }.sum)
        else portfolio
        ...
        case (TracedResult(Right(Result(currDiff, carryFee, currTotalFee, currConstraint)), _), tx) =>
          ...
          txDiffer(currBlockchain, tx).flatMap { newDiff =>
            ...
            if (updatedConstraint.isOverfilled)
              ...
              val totalAcrylFee                = currTotalFee + curBlockFees.balance + nextBlockFee
              ...
              Right(Result(currDiff.combine(resultDiff), resultCarryFee, totalAcrylFee, updatedConstraint))
            }
          }
      }
      .map { result =>
