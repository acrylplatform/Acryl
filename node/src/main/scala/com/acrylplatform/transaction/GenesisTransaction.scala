...
import com.acrylplatform.transaction.TransactionParsers._
import com.acrylplatform.transaction.description.{AddressBytes, ByteEntity, LongBytes}
...
case class GenesisTransaction private (recipient: Address, amount: Long, timestamp: Long, signature: ByteStr) extends Transaction {
  ...
  override val assetFee: (Asset, Long)    = (Acryl, 0)
...
object GenesisTransaction extends TransactionParserFor[GenesisTransaction] with TransactionParser.HardcodedVersion1 {
  ...
  override protected def parseTail(bytes: Array[Byte]): Try[TransactionT] = {
    Try {
      ...
      byteTailDescription.deserializeFromByteArray(bytes).flatMap { tx =>
        Either
          .cond(tx.amount >= 0, tx, TxValidationError.NegativeAmount(tx.amount, "acryl"))
          .foldToTry
      }
    }.flatten
  }
  ...
  def create(recipient: Address, amount: Long, timestamp: Long): Either[ValidationError, GenesisTransaction] = {
    if (amount < 0) {
      Left(TxValidationError.NegativeAmount(amount, "acryl"))
    } else {
