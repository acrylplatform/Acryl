...
import com.acrylplatform.transaction.assets.exchange.Order.assetIdBytes
...
object AssetPair {
  ...
  def assetIdStr(aid: Asset): String = aid match {
    case Acryl           => AcrylName
    case IssuedAsset(id) => id.base58
  }

  def extractAssetId(a: String): Try[Asset] = a match {
    case `WavesName` => Success(Acryl) // TODO : during the transition
    case `AcrylName` => Success(Acryl)
