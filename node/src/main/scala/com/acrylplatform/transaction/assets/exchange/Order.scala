package com.acrylplatform.transaction.assets.exchange
...
import com.acrylplatform.transaction.assets.exchange.Validation.booleanOperators
import com.acrylplatform.utils.byteStrWrites
...
trait Order extends BytesSerializable with JsonSerializable with Proven {
  ...
  def matcherFeeAssetId: Asset = Acryl
  ...
  def getSpendAmount(matchAmount: Long, matchPrice: Long): Either[ValidationError, Long] =
    Try {
      ...
      if (orderType == OrderType.SELL) matchAmount
        ...
        if (getSpendAssetId == Acryl && !(spend + matcherFee).isValidLong) {
  ...
  override def toString: String = {
    val matcherFeeAssetIdStr = if (version == 3) s" matcherFeeAssetId=${matcherFeeAssetId.fold("Acryl")(_.toString)}," else ""
...
object Order {
  ...
  def buy(sender: KeyPair,
          ...
          matcherFee: Long,
          version: Byte = 1,
          matcherFeeAssetId: Asset = Acryl): Order = {
