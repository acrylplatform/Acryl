...
import com.acrylplatform.transaction.{Asset, Proofs}
...
object OrderJson {
  ...
  private val assetReads: Reads[Asset] = {
    case JsNull | JsString("") => JsSuccess(Acryl)
    case JsString(s) =>
  ...
  implicit val assetPairReads: Reads[AssetPair] = {
    val r = (JsPath \ "amountAsset").readWithDefault[Asset](Acryl)(assetReads) and
      (JsPath \ "priceAsset").readWithDefault[Asset](Acryl)(assetReads)
    r(AssetPair(_, _))
  }
