...
import com.acrylplatform.lang.v1.compiler.Terms.{ARR, CaseObj, EVALUATED, FUNCTION_CALL}
import com.acrylplatform.lang.v1.evaluator.ContractEvaluator
import com.acrylplatform.lang.v1.{ContractLimits, FunctionHeader, Serde}
...
object InvokeScriptTransaction extends TransactionParserFor[InvokeScriptTransaction] with TransactionParser.MultipleVersions {
  ...
  def functionCallToJson(fc: Terms.FUNCTION_CALL): JsObject = {
    Json.obj(
      "function" -> JsString(fc.function.asInstanceOf[com.acrylplatform.lang.v1.FunctionHeader.User].name),
      "args" -> JsArray(
        fc.args.map {
    ...
    payments
      ...
      .fold(().asRight[NonPositiveAmount])(
        p =>
          NonPositiveAmount(
            p.amount,
            p.assetId.fold("Acryl")(_.toString)
          ).asLeft[Unit])
  ...
  val byteTailDescription: ByteEntity[InvokeScriptTransaction] = {
    (
      ...
      OptionBytes(tailIndex(7), "Fee's asset ID", AssetIdBytes(tailIndex(7), "Fee's asset ID"), "flag (1 - asset, 0 - Acryl)")
        .map(_.getOrElse(Acryl)),
