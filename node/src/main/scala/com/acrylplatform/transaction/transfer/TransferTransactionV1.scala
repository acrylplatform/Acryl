...
object TransferTransactionV1 extends TransactionParserFor[TransferTransactionV1] with TransactionParser.HardcodedVersion1 {
  ...
  val byteTailDescription: ByteEntity[TransferTransactionV1] = {
    (
      ...
      OptionBytes[IssuedAsset](tailIndex(4), "Asset ID", AssetIdBytes(tailIndex(4), "Asset ID"), "flag (1 - asset, 0 - Acryl)"),
      OptionBytes[IssuedAsset](tailIndex(5), "Fee's asset ID", AssetIdBytes(tailIndex(5), "Fee's asset ID"), "flag (1 - asset, 0 - Acryl)"),
      ...
      case (signature, txId, senderPublicKey, assetId, feeAssetId, timestamp, amount, fee, recipient, attachments) =>
        ...
        TransferTransactionV1(
          assetId = assetId.getOrElse(Acryl),
          sender = senderPublicKey,
          recipient = recipient,
          amount = amount,
          timestamp = timestamp,
          feeAssetId = feeAssetId.getOrElse(Acryl),
          fee = fee,
