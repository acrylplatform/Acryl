...
import com.acrylplatform.common.utils.{Base58, Base64, FastBase58}
...
import com.acrylplatform.{Application, Version}
...
object UtilApp {
  ...
  private[this] lazy val commandParser = {
    ...
    OParser.sequence(
      programName("acryl util"),
      head("Acryl Util", Version.VersionString),
      OParser.sequence(
  ...
  private[this] object Actions {
    ...
    def doSign(c: Command, data: Array[Byte]): ActionResult =
      Right(com.acrylplatform.crypto.sign(c.signOptions.privateKey, data))

    def doVerify(c: Command, data: Array[Byte]): ActionResult =
      Either.cond(com.acrylplatform.crypto.verify(c.verifyOptions.signature, data, c.verifyOptions.publicKey), data, "Invalid signature")
    ...
    def doHash(c: Command, data: Array[Byte]): ActionResult = c.hashOptions.mode match {
      case "fast"   => Right(com.acrylplatform.crypto.fastHash(data))
      case "secure" => Right(com.acrylplatform.crypto.secureHash(data))
