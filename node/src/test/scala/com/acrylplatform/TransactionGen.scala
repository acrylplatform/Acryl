...
import com.acrylplatform.lang.v1.testing.{ScriptGen, TypedScriptGen}
import com.acrylplatform.lang.v1.{ContractLimits, FunctionHeader}
...
import com.acrylplatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, ParsedTransfer}
...
trait TransactionGenBase extends ScriptGen with TypedScriptGen with NTPTime { _: Suite =>
  ...
  val acrylAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]]    = Gen.frequency((1, acrylAssetGen), (10, Gen.option(bytes32gen.map(ByteStr(_)))))

  val assetPairGen: Gen[AssetPair] = assetIdGen.flatMap {
    case None => bytes32gen.map(b => AssetPair(Acryl, IssuedAsset(ByteStr(b))))
    case Some(a1bytes) =>
      ...
      Gen.oneOf(Gen.const(None), a2bytesGen).map { a2 =>
        ...
        val asset2 = a2.fold[Asset](Acryl)(arr => IssuedAsset(ByteStr(arr)))
  ...
  def transferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransactionV1] =
    ...
    } yield TransferTransactionV1.selfSigned(Acryl, sender, recipient, amount, timestamp, Acryl, feeAmount, attachment).explicitGet()

  def transferGeneratorPV2(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransactionV2] =
    for {
      amount                                    <- Gen.choose(1, maxAmount)
      (_, _, _, _, _, _, feeAmount, attachment) <- transferParamGen
    } yield TransferTransactionV2.selfSigned(Acryl, sender, recipient, amount, timestamp, Acryl, feeAmount, attachment).explicitGet()
  ...
  def acrylTransferGeneratorP(sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(sender, recipient, Acryl, Acryl)

  def acrylTransferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(timestamp, sender, recipient, Acryl, Acryl)
  ...
  def createAcrylTransfer(sender: KeyPair,
                          recipient: Address,
                          amount: Long,
                          fee: Long,
                          timestamp: Long): Either[ValidationError, TransferTransactionV1] =
    TransferTransactionV1.selfSigned(Acryl, sender, recipient, amount, timestamp, Acryl, fee, Array())
  ...
  def versionedTransferGenP(sender: PublicKey, recipient: Address, proofs: Proofs): Gen[TransferTransactionV2] =
    (for {
      ...
      fee       <- smallFeeGen
      timestamp <- timestampGen
    } yield TransferTransactionV2.create(Acryl, sender, recipient, amt, timestamp, Acryl, fee, Array.emptyByteArray, proofs).explicitGet())
      .label("VersionedTransferTransactionP")

  val transferWithAcrylFeeGen: Gen[TransferTransactionV1] = for {
    (assetId, sender, recipient, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransactionV1.selfSigned(assetId, sender, recipient, amount, timestamp, Acryl, feeAmount, attachment).explicitGet()

  val selfTransferWithAcrylFeeGen: Gen[TransferTransactionV1] = for {
    (assetId, sender, _, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransactionV1.selfSigned(assetId, sender, sender, amount, timestamp, Acryl, feeAmount, attachment).explicitGet()
  ...
  val invokeScriptGen: Gen[InvokeScriptTransaction] = for {
    ...
    fee       <- smallFeeGen
    timestamp <- timestampGen
  } yield InvokeScriptTransaction.selfSigned(sender, dappAddress.toAddress, Some(fc), po.toSeq, fee, Acryl, timestamp).explicitGet()
  ...
  val exchangeTransactionGen: Gen[ExchangeTransaction] = for {
    ...
    buyerMatcherFeeAssetId  <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, buyerAnotherAsset, Acryl)
    sellerMatcherFeeAssetId <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, sellerAnotherAsset, Acryl)
    r <- Gen.oneOf(
  ...
  def exchangeV2GeneratorP(buyer: KeyPair,
                           ...
                           buyMatcherFeeAssetId: Asset = Acryl,
                           sellMatcherFeeAssetId: Asset = Acryl,
