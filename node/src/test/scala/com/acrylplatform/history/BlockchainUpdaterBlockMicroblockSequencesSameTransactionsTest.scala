...
class BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest
  ...
  property("Miner fee from microblock [Genesis] <- [Empty] <~ (Micro with tx) <- [Empty]") {
    val preconditionsAndPayments: Gen[(KeyPair, GenesisTransaction, TransferTransactionV1, Int)] = for {
      ...
      payment: TransferTransactionV1 = createAcrylTransfer(master, master, amt, fee, ts).explicitGet()
  ...
  def randomPayment(accs: Seq[KeyPair], ts: Long): Gen[TransferTransactionV1] =
    for {
      ...
      fee  <- smallFeeGen
      amt  <- smallFeeGen
    } yield createAcrylTransfer(from, to, amt, fee, ts).explicitGet()
  ...
  def accsAndGenesis(): Gen[(Seq[KeyPair], KeyPair, Block, Int)] =
    for {
      ...
      miner   <- accountGen
      ts      <- positiveIntGen
      genesis1: GenesisTransaction = GenesisTransaction.create(alice, TOTAL_ACRYL / 4, ts).explicitGet()
      genesis2: GenesisTransaction = GenesisTransaction.create(bob, TOTAL_ACRYL / 4, ts + 1).explicitGet()
      genesis3: GenesisTransaction = GenesisTransaction.create(charlie, TOTAL_ACRYL / 4, ts + 2).explicitGet()
      genesis4: GenesisTransaction = GenesisTransaction.create(dave, TOTAL_ACRYL / 4, ts + 4).explicitGet()
    } yield
