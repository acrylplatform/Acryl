...
import com.acrylplatform.account.{KeyPair, Address, AddressOrAlias}
...
class BlockchainUpdaterMicroblockSunnyDayTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransactionV1 <- acrylTransferGeneratorP(master, alice)
    aliceToBob  = createAcrylTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts).explicitGet()
    aliceToBob2 = createAcrylTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts + 1).explicitGet()
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) {
      case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
        ...
        val minerABalance = withDomain(MicroblocksActivatedAt0AcrylSettings) { da =>
        ...
        val minerBBalance = withDomain(MicroblocksActivatedAt0AcrylSettings) { db =>
