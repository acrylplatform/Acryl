...
import com.acrylplatform.features._
...
import com.acrylplatform.settings.{TestFunctionalitySettings, AcrylSettings}
import com.acrylplatform.state.reader.LeaseDetails
import com.acrylplatform.transaction.Asset.{IssuedAsset, Acryl}
import com.acrylplatform.transaction.TxValidationError.AliasDoesNotExist
import com.acrylplatform.transaction.assets.{IssueTransactionV1, ReissueTransactionV1}
...
import com.acrylplatform.transaction.{CreateAliasTransactionV1, DataTransaction, GenesisTransaction, Transaction}
import com.acrylplatform.{NoShrink, TestTime, TransactionGen, history}
...
class RollbackSpec extends FreeSpec with Matchers with WithDomain with TransactionGen with PropertyChecks with NoShrink {
  ...
  private def transfer(sender: KeyPair, recipient: Address, amount: Long) =
    TransferTransactionV1.selfSigned(Acryl, sender, recipient, amount, nextTs, Acryl, 1, Array.empty[Byte]).explicitGet()
      ...
      case 2 =>
        List(
          MassTransferTransaction
            .selfSigned(Acryl, sender, List(ParsedTransfer(recipient, amount), ParsedTransfer(recipient, amount)), nextTs, 10000, Array.empty[Byte])
            .explicitGet())
      case _ => List(TransferTransactionV1.selfSigned(Acryl, sender, recipient, amount, nextTs, Acryl, 1000, Array.empty[Byte]).explicitGet())
    }
  }
  ...
  "Rollback resets" - {
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender, com.acrylplatform.state.diffs.ENOUGH_AMT))
    ...
    "acryl balances" in forAll(accountGen, positiveLongGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
            ...
            TestBlock.create(
              ...
              Seq(
                TransferTransactionV1
                  .selfSigned(IssuedAsset(issueTransaction.id()), sender, recipient, assetAmount, nextTs, Acryl, 1, Array.empty[Byte])
                  .explicitGet())
            ))
    ...
    def createSettings(preActivatedFeatures: (BlockchainFeature, Int)*): AcrylSettings = {
      ...
      history.DefaultAcrylSettings.copy(blockchainSettings = history.DefaultAcrylSettings.blockchainSettings.copy(functionalitySettings = tfs))
    }
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender, com.acrylplatform.state.diffs.ENOUGH_AMT))
