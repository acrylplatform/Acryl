...
class AssetTransactionsDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithDB {
  ...
  property("Issue+Reissue+Burn do not break acryl invariant and updates state") {
  ...
  property("Can burn non-owned alias if feature 'BurnAnyTokens' activated") {
    val setup = for {
      ...
      assetTransfer <- transferGeneratorP(issuer, burner, IssuedAsset(issue.assetId), Acryl)
      acrylTransfer <- acrylTransferGeneratorP(issuer, burner)
      burn = BurnTransactionV1.selfSigned(burner, IssuedAsset(issue.assetId), assetTransfer.amount, acrylTransfer.amount, timestamp).explicitGet()
    } yield (genesis, issue, assetTransfer, acrylTransfer, burn)
    ...
    forAll(setup) {
      case (genesis, issue, assetTransfer, acrylTransfer, burn) =>
        assertDiffAndState(Seq(TestBlock.create(Seq(genesis, issue, assetTransfer, acrylTransfer))), TestBlock.create(Seq(burn)), fs) {
          case (_, newState) =>
  ...
  property("Can not total issue > long.max") {
    val setup = for {
      ...
      transfer = TransferTransactionV1.selfSigned(assetId, issuer, holder, quantity - 1, timestamp, Acryl, fee, attachment).explicitGet()
  ...
  def genesisIssueTransferReissue(code: String): Gen[(Seq[GenesisTransaction], IssueTransactionV2, TransferTransactionV1, ReissueTransactionV1)] =
    for {
      ...
      timestamp          <- timestampGen
      initialAcrylAmount <- Gen.choose(Long.MaxValue / 1000, Long.MaxValue / 100)
      accountA           <- accountGen
      accountB           <- accountGen
      smallFee           <- Gen.choose(1l, 10l)
      genesisTx1 = GenesisTransaction.create(accountA, initialAcrylAmount, timestamp).explicitGet()
      genesisTx2 = GenesisTransaction.create(accountB, initialAcrylAmount, timestamp).explicitGet()
      reissuable = true
      ...
      transfer = TransferTransactionV1
        .selfSigned(assetId, accountA, accountB, issue.quantity, timestamp + 2, Acryl, smallFee, Array.empty)
        .explicitGet()
