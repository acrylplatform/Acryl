...
import com.acrylplatform.account.{AddressScheme, KeyPair, PrivateKey, PublicKey}
...
import com.acrylplatform.state.diffs.ExchangeTransactionDiff.getOrderFeePortfolio
...
class ExchangeTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with Inside with NoShrink {
  ...
  property("Preserves acryl invariant, stores match info, rewards matcher") {
  ...
  property("Preserves assets invariant (matcher's fee in one of the assets of the pair or in Acryl), stores match info, rewards matcher") {
            ...
            val restoredMatcherPortfolio =
              Monoid.combineAll(
                Seq(
                  ...
                  ExchangeTransactionDiff.acrylPortfolio(-exchange.fee)
                )
              )
  ...
  property("buy acryl without enough money for fee") {
    val preconditions: Gen[(GenesisTransaction, GenesisTransaction, IssueTransactionV1, ExchangeTransaction)] = for {
      ...
      exchange <- Gen.oneOf(
        exchangeV1GeneratorP(buyer, seller, Acryl, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000)),
        exchangeV2GeneratorP(buyer, seller, Acryl, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000))
      )
    } yield {
  ...
  property("small fee cases") {
    ...
    forAll(preconditions, priceGen) {
      case ((buyer, seller, matcher, gen1, gen2, issue1), price) =>
        val assetPair = AssetPair(IssuedAsset(issue1.id()), Acryl)
  ...
  property("Diff for ExchangeTransaction works as expected and doesn't use rounding inside") {
    ...
    val assetPair                                          = AssetPair(Acryl, IssuedAsset(issue1.id()))
  ...
  property("Disable use OrderV1 on SmartAccount") {
    ...
    forAll(for {
      ...
      tr1     = createAcrylTransfer(MATCHER, buyer.toAddress, Long.MaxValue / 3, enoughFee, ts + 1).explicitGet()
      tr2     = createAcrylTransfer(MATCHER, seller.toAddress, Long.MaxValue / 3, enoughFee, ts + 2).explicitGet()
