...
import com.acrylplatform.account.{Address, AddressScheme, Alias, KeyPair}
...
import com.acrylplatform.lang.contract.DApp.{CallableAnnotation, CallableFunction}
...
import com.acrylplatform.lang.v1.evaluator.FunctionIds.{CREATE_LIST, THROW}
import com.acrylplatform.lang.v1.evaluator.ctx.impl.acryl.{AcrylContext, FieldNames}
import com.acrylplatform.lang.v1.evaluator.ctx.impl.{CryptoContext, PureContext}
import com.acrylplatform.lang.v1.evaluator.{FunctionIds, ScriptResult}
import com.acrylplatform.lang.v1.parser.{Expressions, Parser}
import com.acrylplatform.lang.v1.{ContractLimits, FunctionHeader, compiler}
...
import com.acrylplatform.transaction.TxValidationError.TransactionNotAllowedByScript
...
import com.acrylplatform.transaction.{Asset, _}
...
class InvokeScriptTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithDB with Inside {
  ...
  def paymentContract(senderBinding: String,
                      ...
                      assets: List[Asset] = List(Acryl)): DApp = {
  ...
  def paymentContractGen(address: Address, amount: Long, assets: List[Asset] = List(Acryl))(func: String): Gen[DApp] =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
    } yield paymentContract(senderBinging, argBinding, func, address, amount, assets)

  def defaultPaymentContractGen(address: Address, amount: Long, assets: List[Asset] = List(Acryl))(someName: String): Gen[DApp] =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
  ...
  def preconditionsAndSetContract(senderBindingToContract: String => Gen[DApp],
    ...
    for {
      ...
      sponsoredFee = Sponsorship.fromAcryl(900000L, sponsorTx.minSponsoredAssetFee.get)
      ...
      ci = InvokeScriptTransaction
        .selfSigned(
          ...
          if (sponsored) sponsorTx.asset else Acryl,
          ts + 3
        )
        .explicitGet()
    ...
    forAll(for {
      ...
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffAndState(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
          case (blockDiff, newState) =>
            blockDiff.scriptsRun shouldBe 1
            newState.balance(acc, Acryl) shouldBe amount
        }
    }
  }
  ...
  property("can't make more than 10 payments") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = (paymentContractGen(a, am, List.fill(11)(Acryl)) _)
      r <- preconditionsAndSetContract(contractGen)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
      ...
      case (acc, amount, genesis, setScript, ci, asset, master, ts) =>
        val t =
          TransferTransactionV2
            .selfSigned(IssuedAsset(asset.id()), master, acc, asset.quantity / 10, ts, Acryl, enoughFee, Array[Byte]())
            .explicitGet()
  ...
  property("payment should be positive") {
    forAll(for {
      ...
      ci = InvokeScriptTransaction.selfSigned(invoker, master, Some(fc), Seq(Payment(-1, Acryl)), fee, Acryl, ts)
  ...
  property("can't overflow payment + fee") {
    forAll(for {
      ...
      r <- preconditionsAndSetContract(contractGen,
                                       invokerGen = Gen.oneOf(Seq(invoker)),
                                       payment = Some(Payment(Long.MaxValue, Acryl)),
                                       feeGen = ciFee(1))
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("can't overflow sum of payment in contract") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = (paymentContractGen(a, Long.MaxValue / 2 + 2, List.fill(4)(Acryl)) _)
      invoker <- accountGen
      ts      <- timestampGen
      r       <- preconditionsAndSetContract(contractGen, invokerGen = Gen.oneOf(Seq(invoker)), payment = Some(Payment(1, Acryl)), feeGen = ciFee(1))
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
          _ should produce("Attempt to transfer unavailable funds")
  ...
  property("invoking contract with sponsored fee") {
    forAll(for {
      ...
      case (ts, acc, amount, genesis, setScript, ci, master, sponsoredAsset, setSponsorship) =>
        val t =
          TransferTransactionV2
            .selfSigned(IssuedAsset(sponsoredAsset.id()), master, ci.sender, sponsoredAsset.quantity / 10, ts, Acryl, enoughFee, Array[Byte]())
            .explicitGet()
  ...
  property("Default function invocation should produce error if contract default function has arguments") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "default", a, am, List(Acryl))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("Default function invocation should produce error if contract does't have default function") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "undefault", a, am, List(Acryl))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
