...
import com.acrylplatform.lang.v1.compiler.Terms.CONST_BYTESTR
import com.acrylplatform.lang.v1.evaluator.ctx.impl._
    ...
    forAll(transferV1Gen) {
      case transfer =>
        ...
        transfer.assetId match {
          ...
          case Acryl          => result should produce("extract() called on unit")
        }
    }
  }
  ...
  property("isDefined should return true if transfer assetId exists") {
    forAll(transferV1Gen) {
      case transfer =>
        ...
        result shouldEqual evaluated(transfer.assetId != Acryl)
    }
  }
