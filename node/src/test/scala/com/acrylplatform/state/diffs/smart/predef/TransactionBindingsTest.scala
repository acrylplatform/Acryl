...
import com.acrylplatform.lang.Testing.evaluated
...
import com.acrylplatform.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_BYTESTR, CONST_LONG, CONST_STRING, EVALUATED}
...
import com.acrylplatform.transaction.{Proofs, ProvenTransaction, VersionedTransaction}
import com.acrylplatform.utils.EmptyBlockchain
import com.acrylplatform.{NoShrink, TransactionGen, crypto}
...
class TransactionBindingsTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
    ...
    forAll(invokeScriptGen) { t =>
      ...
      val script =
        s"""
            ...
            |     then if (${t.payment.headOption.exists(_.assetId != Acryl)})
  ...
  property("MassTransferTransaction binding") {
    forAll(massTransferGen) { t =>
      ...
      val script = s"""
                      ...
                      |    let assetId = if (${t.assetId != Acryl}) then extract(t.assetId) == base58'${t.assetId.maybeBase58Repr
                        .getOrElse("")}'
                      |      else isDefined(t.assetId) == false
    ...
    forAll(exchangeTransactionGen) { t =>
      def pg(ord: Order) = {
        ...
        val script = s"""
           ...
           |   let ${oType}AssetPairAmount = if (${ord.assetPair.amountAsset != Acryl}) then extract(t.${oType}Order.assetPair.amountAsset) == base58'${ord.assetPair.amountAsset.maybeBase58Repr
                          .getOrElse("")}'
           |   else isDefined(t.${oType}Order.assetPair.amountAsset) == false
           |   let ${oType}AssetPairPrice = if (${ord.assetPair.priceAsset != Acryl}) then extract(t.${oType}Order.assetPair.priceAsset) == base58'${ord.assetPair.priceAsset.maybeBase58Repr
                          .getOrElse("")}'
           |   else isDefined(t.${oType}Order.assetPair.priceAsset) == false
           |   let ${oType}MatcherFeeAssetId = if (${ord.matcherFeeAssetId != Acryl}) then extract(t.${oType}Order.matcherFeeAssetId) == base58'${ord.matcherFeeAssetId.maybeBase58Repr
                          .getOrElse("")}'
  ...
  property("Order binding") {
    forAll(orderGen) { t =>
      val s = s"""
                 ...
                 |   ${Range(0, 8).map(letProof(t.proofs, "t")).mkString("\n")}
                 |   let assetPairAmount = if (${t.assetPair.amountAsset != Acryl}) then extract(t.assetPair.amountAsset) == base58'${t.assetPair.amountAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.amountAsset) == false
                 |   let assetPairPrice = if (${t.assetPair.priceAsset != Acryl}) then extract(t.assetPair.priceAsset) == base58'${t.assetPair.priceAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.priceAsset) == false
                 |   let matcherFeeAssetId = if (${t.matcherFeeAssetId != Acryl}) then extract(t.matcherFeeAssetId) == base58'${t.matcherFeeAssetId.maybeBase58Repr
                   .getOrElse("")}'
  ...
  def runForAsset(script: String): Either[String, EVALUATED] = {
    ...
    import com.acrylplatform.lang.v1.CTX._
      ...
      PureContext
        ...
        AcrylContext
          .build(
            ...
            new AcrylEnvironment(chainId, Coeval(null), null, EmptyBlockchain, Coeval(null))
          )
      ...
      PureContext.build(Global, V2) |+|
        ...
        AcrylContext
          .build(
            ...
            new AcrylEnvironment(chainId, Coeval(t), null, EmptyBlockchain, Coeval(null))
          )
