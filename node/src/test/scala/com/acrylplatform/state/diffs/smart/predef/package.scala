...
package object predef {
  ...
  def scriptWithAllV1Functions(tx: DataTransaction, t: TransferTransaction): String =
    s"""${dropLastLine(scriptWithV1PureFunctions(tx, t))}
       |${dropLastLine(scriptWithV1AcrylFunctions(tx, t))}
       |${dropLastLine(scriptWithCryptoFunctions)}
       |if rnd then pure && acryl else crypto""".stripMargin
  ...
  def scriptWithV1AcrylFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s""" # Acryl context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && acrylBalance(tx.sender) != 0
       |
       | let acryl = txById && entries && balances && aFromPK && aFromStrOrRecip && height > 0
       | acryl""".stripMargin
