...
import com.acrylplatform.account.{KeyPair, AddressOrAlias}
import com.acrylplatform.common.utils.EitherExt2
import com.acrylplatform.lagonaki.mocks.TestBlock
import com.acrylplatform.lang.v1.compiler.Terms.{CONST_BYTESTR, CaseObj}
import com.acrylplatform.state.diffs._
import com.acrylplatform.state.diffs.smart.predef._
import com.acrylplatform.transaction.Asset.Acryl
import com.acrylplatform.transaction.transfer._
import com.acrylplatform.transaction.{CreateAliasTransaction, GenesisTransaction}
...
class AddressFromRecipientScenarioTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
  ...
  val preconditionsAndAliasCreations: Gen[(Seq[GenesisTransaction], CreateAliasTransaction, TransferTransactionV1, TransferTransactionV1)] = for {
    ...
    transferViaAddress <- transferGeneratorP(master, other, Acryl, Acryl)
    transferViaAlias   <- transferGeneratorP(master, AddressOrAlias.fromBytes(alias.bytes.arr, 0).explicitGet()._1, Acryl, Acryl)
