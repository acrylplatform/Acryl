...
import com.acrylplatform.history
...
class BlockchainUpdaterTest extends FreeSpec with Matchers with HistoryTest with ShouldVerb with WithDomain {
  ...
  private val AcrylSettings = history.DefaultAcrylSettings.copy(
    blockchainSettings = history.DefaultAcrylSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultAcrylSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultAcrylSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val AcrylSettingsWithDoubling = AcrylSettings.copy(
    blockchainSettings = AcrylSettings.blockchainSettings.copy(
      functionalitySettings = AcrylSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(AcrylSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(AcrylSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(AcrylSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(AcrylSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(AcrylSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(AcrylSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(AcrylSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(AcrylSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(AcrylSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
