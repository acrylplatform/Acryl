...
abstract class GenericTransactionSpecification[T <: com.acrylplatform.transaction.Transaction]
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen {

  def transactionParser: com.acrylplatform.transaction.TransactionParserFor[T]
  def updateProofs(tx: T, p: Proofs): T
  def generator: Gen[((Seq[com.acrylplatform.transaction.Transaction], T))]
