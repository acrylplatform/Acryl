...
import com.acrylplatform.transaction.assets.exchange.{AssetPair, Order, OrderType, _}
  ...
  property("AssetPair test") {
    forAll(assetIdGen, assetIdGen) { (assetA, assetB) =>
      whenever(assetA != assetB) {
        val assetAId: Asset = assetA.fold[Asset](Acryl)(arr => IssuedAsset(arr))
        val assetBId: Asset = assetB.fold[Asset](Acryl)(arr => IssuedAsset(arr))
