...
import com.acrylplatform.lang.v1.evaluator.ctx.impl.acryl._
...
class UserFunctionComplexityTest extends PropSpec with PropertyChecks with Matchers with TypedScriptGen {
  ...
  property("estimate script for stdLib V1 with UserFunctions") {
    ...
    val exprAcrylBalance = FUNCTION_CALL(User("acrylBalance"), List(CONST_STRING("alias").explicitGet()))
    est(exprAcrylBalance).explicitGet() shouldBe 110
  }
