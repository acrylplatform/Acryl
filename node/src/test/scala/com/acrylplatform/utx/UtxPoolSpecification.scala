...
import com.acrylplatform.db.{WithDomain, openDB}
import com.acrylplatform.features.BlockchainFeatures
import com.acrylplatform.history.{StorageFactory, randomSig}
...
import com.acrylplatform.lang.v1.compiler.{CompilerContext, ExpressionCompiler}
import com.acrylplatform.mining._
...
import com.acrylplatform.transaction.{Asset, Transaction, _}
...
class UtxPoolSpecification
  ...
  private def mkBlockchain(senderAccount: Address, senderBalance: Long) = {
    ...
    val origSettings    = AcrylSettings.fromRootConfig(config)
  ...
  private def transfer(sender: KeyPair, maxAmount: Long, time: Time) =
    ...
    } yield TransferTransactionV1.selfSigned(Acryl, sender, recipient, amount, time.getTimestamp(), Acryl, fee, Array.empty[Byte]).explicitGet())
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createAcrylTransfer(sender, recipient, amountPart, fee, time.getTimestamp()).explicitGet()
  ...
  private def transactionV1Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransactionV1] = accountGen.map { recipient =>
    TransferTransactionV1.selfSigned(Acryl, sender, recipient, acryl(1), ts, Acryl, feeAmount, Array.emptyByteArray).explicitGet()
  }

  private def transactionV2Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransactionV2] = accountGen.map { recipient =>
    TransferTransactionV2.selfSigned(Acryl, sender, recipient, acryl(1), ts, Acryl, feeAmount, Array.emptyByteArray).explicitGet()
  }
  ...
  "UTX Pool" - {
    ...
    "correctly process constrainst in packUnconfirmed" in {
      withDomain(acrylplatform.history.TransfersV2ActivatedAt0AcrylSettings) { d =>
