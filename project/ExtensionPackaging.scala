...
object ExtensionPackaging extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[_]] =
    Seq(
      ...
      // To write files to Acryl NODE directory
  ...
  private def maintainerFix = inConfig(Linux)(
    Seq(
      ...
      packageSummary := s"Acryl node ${name.value}${network.value.packageSuffix} extension",
      packageDescription := s"Acryl node ${name.value}${network.value.packageSuffix} extension"
    ))
